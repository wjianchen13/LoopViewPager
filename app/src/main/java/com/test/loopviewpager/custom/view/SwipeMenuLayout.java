    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childView.getVisibility() != GONE) {
                ...
                measureChild1(childView, widthMeasureSpec, heightMeasureSpec);
        ...
        if (isNeedMeasureChildHeight) {//如果子View的height有MatchParent属性的，设置子View高度
            // 这里为什么需要这样设置，因为每个子View的高度不一样，把有match_parent的子view高度设置成和父布局一样的高度
            ...
            if (childView.getVisibility() != GONE) {
                if (i == 0) {//第一个子View是内容 宽度设置为全屏
                    ...
                    if (isLeftSwipe) {
                        childView.layout(left, getPaddingTop(), left + childView.getMeasuredWidth(), getPaddingTop() + childView.getMeasuredHeight());
                        left = left + childView.getMeasuredWidth();
                    } else { // 这里不知道有没有错误，是不是应该left = 0 - childView.getMeasuredWidth()，需要测试左滑删除才知道
...
/*    @Override
    ...
    public void setmSwipeListener(ISwipeListener mSwipeListener) {
    ...
    protected void measureChild1(View child, int parentWidthMeasureSpec,
                                 int parentHeightMeasureSpec) {
        final LayoutParams lp = child.getLayoutParams();

        final int childWidthMeasureSpec = getChildMeasureSpec1(parentWidthMeasureSpec,
                getPaddingLeft() + getPaddingRight(), lp.width);
        final int childHeightMeasureSpec = getChildMeasureSpec1(parentHeightMeasureSpec,
                getPaddingTop() + getPaddingBottom(), lp.height);

        child.measure(childWidthMeasureSpec, childHeightMeasureSpec);
    }

    public static int getChildMeasureSpec1(int spec, int padding, int childDimension) {
